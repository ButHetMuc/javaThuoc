...
//		// Không chứa ký số hoặc các ký tự đặc biệt khác, ngoại trừ ký tự ‘
		if (!(ma.length() > 0 && ma.matches("^NV\\d{2}$"))) {
			showMessage("Error: mã nhân viên phải bắt đầu là NV theo sau là 2 số", txtMaNV);
			return false;
		}
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (o.equals(btnLuu) && btnSua.isEnabled()){
				...
				if (nv_dao.update(nv)){
					...
					dataModel.setValueAt(sqlDate,i,5);
					dataModel.setValueAt(phai,i,6);
					dataModel.setValueAt(gia,i,7);
					dataModel.setValueAt(caLam,i,8);
				}
			} catch (Exception e2) {
				System.out.println(e2);
			}
